...
export const getServerSideProps: GetServerSideProps<UsersPageProps> = async (context) => {
  ...
  // Wait for async users to resolve
  const asyncUsers = await result.asyncUsersPromise;
...
export default function UsersPage({ syncUsers: initialSyncUsers, asyncUsers: initialAsyncUsers }: UsersPageProps) {
  ...
  return (
    <article>
      <NextMetaHead meta={DEFAULT_META} />
      <PageHeader>
        <span>⌂</span>
      </PageHeader>
      <div id="homepage-columns">
        ...
        <section className='short'>
          ...
          <details>
            ...
            <div className="explanation">
              After <a href="https://twitter.com/_max_bo_/status/1815536378522022130">initial challenges</a> with preserving camera state when navigating between pages, the app has been successfully ported to <a href="https://nextjs.org/">Next.js</a> with server-side rendering.
              <br />
              <br />
              The migration enables server-side generation of OpenGraph metadata using Firebase data, which was previously handled by Netlify edge functions.
            </div>
          </details>
        </section>
      </div>
...
function UserListEntry(props: { user: User }) {
  ...
  return (
    <li key={user.id}>
      <NextQueryPreservingLink to={user.id}>{user.name}</NextQueryPreservingLink> <Size ts={user.collections} t="collection" />
      <ul>
        {
