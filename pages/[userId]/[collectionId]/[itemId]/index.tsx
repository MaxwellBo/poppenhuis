import { Collection, Item, ITEM_FIELD_SCHEMAS, User } from "../../../../src/manifest";
...
export const getServerSideProps: GetServerSideProps<ItemPageProps> = async (context) => {
  ...
  try {
    const { item, user, collection, users } = await loadItem({ params: { userId, collectionId, itemId }, request });
    return {
      props: {
        item,
        user,
...
export default function ItemPage({ item, user, collection, users }: ItemPageProps) {
  ...
  const vrMode = router.query.vr as string || "";
  ...
  const handlePositioningChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const query = { ...router.query, vr: e.target.value };
    router.push({ pathname: router.pathname, query }, undefined, { shallow: true });
  };

  return (
    <article className='item-page'>
      <NextMetaHead meta={metaForItem(item, collection, user)} />
      <PageHeader>
        <NextQueryPreservingLink to={`/${user.id}`}>{user.name}</NextQueryPreservingLink> / <NextQueryPreservingLink to={`/${user.id}/${collection.id}`}>{collection.name}</NextQueryPreservingLink> / {item.name} <span className='index'>({collection.items.indexOf(item) + 1})</span>
      </PageHeader>
      <div className='bento'>
        <div id="previous">
          <ItemCard
...
function QrCodeAndLinksAndButtons(props: { item: Item; collection: Collection; user: User; modelViewerRef: React.RefObject<HTMLElement>; }) {
  ...
  return (
    <div className="qrcode-and-links-and-buttons">
      ...
      <div id="links">
        <NextQueryPreservingLink className="action-link" to={`/${user.id}/${collection.id}/${item.id}/label`}>print label</NextQueryPreservingLink>, <NextQueryPreservingLink className="action-link" to={`/${user.id}/${collection.id}/${item.id}/embed`}>embed</NextQueryPreservingLink>, <QuicklookLink item={item} />, <a href={githubManifestCodeSearchUrl}>source</a>{user.source === undefined && <>, <a href={editYamlUrl}>edit?</a></>}{user.source === 'firebase' && <>, <NextQueryPreservingLink to={`/${user.id}/${collection.id}/${item.id}/edit`}>edit?</NextQueryPreservingLink></>}
      </div>
      <div id="buttons">
