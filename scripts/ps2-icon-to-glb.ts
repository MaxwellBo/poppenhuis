...
function batch(root: string) {
  ...
  for (const file of files) {
    ...
    for (const entry of loaded) {
      ...
      const outPath = join(dirname(file), `${id}.glb`);
      try {
      ...
      console.log(`${entry.icon.animationShapes > 1 ? 'anim' : 'stat'} ${id.padEnd(40)} ${entry.icon.animationShapes}sh ${entry.icon.vertexCount}v  ${name}`);
    }
  }

  console.log(`\nWrote ${converted} GLBs next to their source files.`);
  console.log('The PS2 save-icon collection is generated by scripts/import-ps2iodb.ts.');
}
